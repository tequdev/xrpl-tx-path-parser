...
const getOfferChangesAmount = (tx: TxResponse<Payment>['result']) => {
  return extractExchanges(tx, { collapse: true }) as Record<'takerPaid' | 'takerGot', Balance>[]
}

const getAmmAccounts = (meta: PaymentMetadata): string[] => {
...
const getAccountBalanceChanges = (meta: PaymentMetadata) => {
...
const amountToBalance = (amount: Amount): Balance => {
...
export const pathParser = (tx: TxResponse['result']) => {
  ...
  if (tx.TransactionType !== 'Payment' || !tx.meta.delivered_amount) throw new Error('Invalid transaction type')
